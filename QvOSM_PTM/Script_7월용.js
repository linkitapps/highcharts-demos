...
var showRoute;
var routeObj={};
var _ty_obj = [];//태풍에 영향받는 mmsi_no를 담는다
var typoonObj={};
var typhoon_info = [];
var portObj={};
var broken_aniarr=[];
...
Qva.LoadScript(QvOSM_PVM_exUrl+"js/d3.v3.min.js", function(){
	Qva.LoadScript(QvOSM_PVM_exUrl+"js/ol.js", function(){
		...
		var createIconStyle = function(icon, rotation, scale, name, offx, offy,fontstyle) {
		...
		var createTyphoonIconStyle = function(icon, rotation, scale, name, offx, offy,fontstyle) {
		...
		Qva.AddExtension(ExName, function() {
			...
			if(!varsRetrieved){
				...
				try {
					...
					var portNames = [];
                     //vTY_STATUS
                     if(IB02.GetVariable(9).text){
                    	 //ty_obj = JSON.parse('[{"NAME_ENG":"CEBILE","MMSI_NO":"218812000","TY_STATUS":"R"},{"NAME_ENG":"CEBILE","MMSI_NO":"565323000","TY_STATUS":"R"},{"NAME_ENG":"CEBILE","MMSI_NO":"566093000","TY_STATUS":"R"},{"NAME_ENG":"CEBILE","MMSI_NO":"636016986","TY_STATUS":"R"},{"NAME_ENG":"CEBILE","MMSI_NO":"636092806","TY_STATUS":"R"},{"NAME_ENG":"FEHI","MMSI_NO":"218024000","TY_STATUS":"Y"},{"NAME_ENG":"FEHI","MMSI_NO":"229572000","TY_STATUS":"Y"},{"NAME_ENG":"FEHI","MMSI_NO":"564386000","TY_STATUS":"Y"}]');
                    	 _ty_obj = JSON.parse(IB02.GetVariable(9).text);
                     }
                   ...
                   //vPATH
					...
					if(IB02.GetVariable(1).text){
						...
						if(typoonObj.length>0){
							...
							for(var ti=0;ti<typoonObj.length;ti++){
								...
								//typhoon rader :: 괘적은 다음에
								if(typoonObj[ti].LATEST_YN == "") continue;
								if(key_typhoon=="") key_typhoon = typoonObj[ti].KEY_TYPHOON;
								if(key_typhoon == typoonObj[ti].KEY_TYPHOON){
									
									if(typoonObj[ti].LATEST_YN == "Y"){
									...
									dbl_ty = 0;
									key_typhoon = typoonObj[ti].KEY_TYPHOON;
									if(typoonObj[ti].LATEST_YN == "Y"){
										if(dbl_ty==0){
											dbl_ty = 1;
										}else{
								...
								typhoon_info.push(typoonObj[ti]);
								...
								var tyPos = {
									...
									speed : typoonObj[ti].MAX_WINDSPEED,
					...
					if($target.Layout.Text2 != null){
						if($target.Layout.Text2.text == "1"){
							weatherFlag = true;
							weatherLayerDs = "";
						}
					}
					...
					if($target.Layout.Text13 != null){
						if($target.Layout.Text13.text != ""){
							switch($target.Layout.Text13.text.toLowerCase()) {
								case "satellite":
									defMap = "layersTn";
									break;
								case "dark":
					...
					if(dataCnt === 9){
						...
						$(rows).each(function(i){
							...
							if(type === "VESSEL"){
								...
								$.each(_ty_obj,function($key, $data){
									if(mmsi==$data["MMSI_NO"]){
										switch($data["TY_STATUS"]){
										case "R": //red
											inout = inout.replace("MOVING","BROKEN");
											break;
										case "Y": //yellow
											inout = inout.replace("MOVING","NOTMOVING");
											break;
										};
									}
								});
					...
					if(!$target.framecreated){
						...
						//if(weatherFlag){
							htmlString += '<div id="weatherLayer1" class="mapLayer1 weatherArea" style="display:none;float:right;right:30px;top:20px;width:130px;color:#ffffff">'+
										...
										'</div>';
						//};
						*//*<img src="'+QvOSM_PVM_exUrl+'icon/btn_transit.png" width="89" height="27"/>*/
						htmlString += '<div id="pointPopup" class="ol-popup"><div class="ol-popup-title"><span class="ol-popup-headtitle-content"></span><span class="ol-popup-title-content"></span></div><a href="#" id="popup-closer" class="ol-popup-closer" style="cursor:pointer;"></a><div id="popup-content"></div></div><div id="geo-marker"></div>'+
										'<div id="'+uId+'" class="map" style="width:'+frmW+'px;height:'+frmH+'px;"></div>'+
									  '</div>';
						
						//typhoon rader :: port disabled
						/*
						htmlString += '<div class="port-encheck">'+
											'<img class="port-encheck-img" src="'+QvOSM_PVM_exUrl+'icon/i_left03.png" width="64" height="73"/>'+
									  '</div>';
						  */
						...
						htmlString += '<div class="weather-encheck">'+
											'<img class="weather-encheck-img" src="'+QvOSM_PVM_exUrl+'icon/i_left_weather.png" width="64" height="73"/>'+
									  '</div>';
						 */
						htmlString += '<div id="sProgressPopup">'+
										'<a href="#" id="sProgressPopup-closer" class="sProgressPopup-closer" style="cursor:pointer;"></a>'+
										'<div class="form-group">'+
											'<div class="col-sm-10">'+
						...
						*/
						...
						QvOSM_PVM_MAP.on("singleclick", function (evt) {
							...
							var info = QvOSM_PVM_MAP.forEachFeatureAtPixel(evt.pixel, function (feature, layer) {
								...
								}else if(feature.get("type")=="vessel"){
									//console.log(feature);
								           ...
								           if(!varsRetrieved){
								                ...
								                Qv.GetDocument("").GetAllVariables(function(variables){
								                     ...
								                     for(var i = 0; i < variables.length; i++){
								                    	 //typhoon rader :: 배정보 클릭시 MMSI_NO를 vMMSI_NO에 대입
								                    	 if(variables[i].name == "vMMSI_NO"){
								                    		 console.log(feature);
								                    		 console.log(variables[i].name + " = " + variables[i].value);
								                    		 break;
								                    	 }
								...
								}else if(feature.get("type")=="typhoon"){
									
									//typhoon rader :: 태풍 클릭 이벤트는 없음
									return;
									
									//console.log(feature);
									$(".ol-popup-headtitle-content").html("<img style='width:20px;' src='"+QvOSM_PVM_exUrl+"icon/typhoon_sky.png"+"'/>");
									$(".ol-popup-title-content").text(feature.get("tyKey"));
							...
							if(info!=null && info.get("popup")){
								pointPopupContent.innerHTML = '<p class="popup_content">'+info.get("popup")+'</p>';
					...
					*/
					...
					if(QvOSM_PVM_Opt["typhoon"]){
						...
						//typhoon rader :: polygone style disable 폴리곤 그려야 할때 아래 주석 해제
						//if(1==2)
						...
						//태풍 대신 폴리곤이 필요할 경우 태풍 아이콘을 그리지 않을때 아래 주석 해제
						//if(1==2)
						for(;tyi_i < tyPosLength; tyi_i++){
							...
							tyIconFea.set('type',  "typhoon");
							tyIconFea.set('speed',  typhoonData.tyPos[tyi_i].speed);
							tyIconFea.set('popup',  typhoonData.tyPos[tyi_i].popup);
							//icon, rotation, scale, name, offx, offy
							
							if(tyDateLabelC){
								//typhoon rader :: 태풍이름 출력
								//tyIconFea.set('style', createIconStyle(typhoonData.tyPos[tyi_i].icon, null, null, typhoonData.tyPos[tyi_i].tyDate, 30, offy));
								
								var fontstyle = '18px Calibri,sans-serif';
								//tyIconFea.set('style', createIconStyle(typhoonData.tyPos[tyi_i].icon, null, null, typhoonData.tyPos[tyi_i].typhoonKey, 25, 1,fontstyle));
								tyIconFea.set('style', createTyphoonIconStyle(typhoonData.tyPos[tyi_i].icon, null, null, typhoonData.tyPos[tyi_i].typhoonKey, 25, 1,fontstyle));
							} else {
								tyIconFea.set('style', createTyphoonIconStyle(typhoonData.tyPos[tyi_i].icon));
							}
							
							
							offy = (offy === 15) ? 22: 15;
							
							//console.log(tyIconFea);
							
							//태풍의 영향도 애니메이션이 필요할때 아래 함수를 사용
							typhoon_flash(tyIconFea,tyi_i);
							...
							if(i<routeObjLength-1){
								...
								if(arcCoord.length > 1){
									...
									for(j = 0; j<arcCoordLength; j++){
										...
										}else if((arcCoordstyle[0]!="D"&&arcCoordstyle[arcCoordLength]=="N")){
											routeFea.push(
												new ol.Feature({
													type: 'route_ing',
													geometry: new ol.geom.MultiLineString(lineCoord)
					...
					if(IB02.GetVariable(2).text=="Y"){
						for(var vesslei=0;vesslei<routeOverlay.length;vesslei++){
							//console.log(routeOverlay[routei]);
							QvOSM_PVM_MAP.removeOverlay(vesselLabelOverlay[vesslei]);
						}
						routeDraw();
					...
					$("#ty_img_div").remove();
					$(".Document_TX128").append('<div id="ty_img_div" style="position:absolute;left:  10px;top: 80px;text-align:center;">'
							+'<img src="'+QvOSM_PVM_exUrl+'icon/typhoon2.png"/>'
						+'</div>');


					$("#typhoon_grade_div").remove();
					$(".Document_TX128").append('<div id="typhoon_grade_div" style="position:absolute;left:  258px;top: 0px;color:#FFFFFF;cursor:pointer;font-weight:bold;">Typhoon Grade <font style="font-size:16px;">ⓘ</font>'
						+'</div><div id="typhoon_grade" style="display:none;position:absolute;left:  400px;top: 0px;">'
						+'<table border=1 borderColor="#FFFFFF" style="background-color:white;width:300px;text-align:center;" align=center>'
						+'<tr><td align=center style="background:#ababab;color:#000000;font-weight:bold;">Grade</td><td align=center style="background:#ababab;color:#000000;font-weight:bold;">Max Wind Speed</td></tr>'
						+'<tr><td align=center style="background:#CCCCCC;color:#000000;font-weight:bold;">Very Strong</td><td align=center style="background:#CCCCCC;color:#000000;">85kn~</td></tr>'
						+'<tr><td align=center style="background:#FFFFFF;color:#000000;font-weight:bold;">Strong</td><td align=centerr style="background:#FFFFFF;color:#000000;">64∼85 kn</td></tr>'
						+'<tr><td align=center style="background:#CCCCCC;color:#000000;font-weight:bold;">Middle</td><td align=centerr style="background:#CCCCCC;color:#000000;">48~64 kn</td></tr></tr>'
						+'<tr><td align=center style="background:#FFFFFF;color:#000000;font-weight:bold;">Weak</td><td align=center style="background:#FFFFFF;color:#000000;">34~48 kn</td></tr></table></div>');	
					$("#typhoon_grade_div").off("click").on("click",function(){
						if($("#typhoon_grade").attr("show")==1){
							$("#typhoon_grade").attr("show",0).hide();
						}else{
							$("#typhoon_grade").attr("show",1).show();
						}
					});
					...
					$("#typhoon_div_wrapper").remove();
					$("#typhoon_div").append('<div id="typhoon_div_wrapper_top" style="font-family:Calibri;font-size:22px;font-weight:bold;color:white;"><div style="border-bottom:1px solid #FFFFFF;padding-bottom:2px;width:105px;">Typhoon <font style="font-size:16px;">ⓘ</font></div></div><div id="typhoon_desc_wrapper" style="width:100%;height:240px;overflow-x:hidden;overflow-y:auto;"><div id="typhoon_desc" style="width:100%;"></div></div>');
					
					$.each(typhoon_info,function($key,$data){
						$('#typhoon_desc .typhoon_info[target="'+$data["KEY_TYPHOON"]+'"]').remove();
						$("#typhoon_desc").append('<div class="typhoon_info" target="'+$data["KEY_TYPHOON"]+'" style="padding-top:5px;"><div id="ty_title" style="color:#006ca2;font-size:18px;font-family:Calibri;font-weight:bold;">'+$data["KEY_TYPHOON"]+'</div><div id="ty_cont" style="color:#FFFFFF;font-size:14px;font-family:Calibri;">'+$data["DESC"]+'</div></div>');
					});
