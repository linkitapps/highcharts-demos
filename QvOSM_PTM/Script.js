...
//태평양 처리 2019.04.11
function fn_pacificOceanRootModify(datarow) {
    for (var i = 0; i < datarow.length; i++) {

        if (i > 0) {
            if ((datarow[i][0] * datarow[i - 1][0] < 0) && datarow[i][0] < 0) {
                var startPoint = datarow[i - 1][0]; //+
                var endPoint = datarow[i][0]; //									
                var cPoint = startPoint / 2 + (endPoint < 0 ? -90 : 90);

                if (!(startPoint > cPoint && endPoint > cPoint) && !(startPoint < cPoint && endPoint < cPoint)) {
                    datarow[i][0] = 360 + datarow[i][0];
                }

            } else if ((datarow[i][0] * datarow[i - 1][0] < 0) && datarow[i][0] > 0) {
                var startPoint = datarow[i - 1][0]; //+
                var endPoint = datarow[i][0]; //									
                var cPoint = startPoint / 2 + (endPoint < 0 ? -90 : 90);
    ...
    for (var k = 0; k < datarow.length; k++) {
        route_arr.push(ol.proj.fromLonLat(datarow[k]));
    }
...
function getroutepathCoord(datarow) {
    ...
    $.each(datarow, function($k, $v) {
        ...
        if ($v[0] < 0) r360_arr.push(ol.proj.fromLonLat([360 + $v[0], $v[1]]));
        else r360_arr.push(ol.proj.fromLonLat($v));
...
var drawLine = function(id, obj, route_arr, width, zoomlevel, linecolor, linestyle, descript, offsetX, offsetY, textAlign, fontstyle, fontcolor) {
    ...
    //console.log( 'route_arr : ', route_arr );
...
var addLayer = function($target, indx) {
    ...
    try {
		if(indx)
			LAYER_OBJ[$target].setZIndex(indx);
        ...
        $.each(vTHRE_VESSEL_DATA_LOC, function(key, value) {
            ...
            $.each(vTHRE_VESSEL_DATA_LOC, function(dataKey, dataValue) {
                ...
                $.each(dataValue, function($key, $value) {
                    ...
                    $value['popup'] = "MMSI : " + $value["MMSI_NO"] + "<br/>" //"Vessel Name : "+$value["VSL_NAME"]+"<br/>"
                        +
                        "Lat : " + $value["LTITDE"] + "<br/>" +
                        "Lon : " + $value["LNGTD"] + "<br/>" +
                    ...
                    if (dataValue.length === 1) { // 좌표가 한개면 그냥 배 아이콘만 표시 되겠지? 점과 점이 연결되어야 선인데. 점 하나 밖에 없으니깐.
                        ...
                        if ($key === dataValue.length - 1) {
                            ...
                            drawScheduledVesselRoute($value['ROUT_CD'] + "_schedule_vessel_dash_" + $value['MMSI_NO'], $value, getroutepathCoord([$value["START_POINT"], dataValue[$key + 1]["START_POINT"]]), null, 'dash', 'rgb(79, 73, 73, 0.5)', 3, null);
                        }
                    }
...
var drawTyphoonVessel = function() {
    ...
    if (vTY_STATUS[$_VAR["vSEL_TYPHOON"]]) {
        //console.log(vTY_STATUS[$_VAR["vSEL_TYPHOON"]]);
...
var drawSelectTyphoonVessel = function($key_sel) {
    if (vesselLayer) QvOSM_PVM_MAP.removeLayer(vesselLayer);
...
var drawSelectPortVessel = function() {
    if (vesselLayer) QvOSM_PVM_MAP.removeLayer(vesselLayer);
    $.each(vPORT_STATUS, function($key, $value) { //vessel
        //console.log('$key',$key);
        //console.log('$value',$value);
        var point = ol.proj.fromLonLat($value["point"]);
		if(vPORT_SELECT) {
			drawLine($value["nx_port_cd"] + "_vsl_name_" + $key, $value, point, 0.5, null, null, "text", $value["vsl_name"], null, null, null, "bold 14px Calibri,sans-serif", "rgba(255,153,255,1)");
			addLayer($value["nx_port_cd"] + "_vsl_name_" + $key, 1210);
		}
        drawLine($value["nx_port_cd"] + "_vessel_icon_" + $key, $value, point, 1, null, null, "icon");
        addLayer($value["nx_port_cd"] + "_vessel_icon_" + $key, 1210);
    });
};
    ...
    Qva.LoadScript(QvOSM_exUrl + "js/perfect-scrollbar.js", function() {
        Qva.LoadScript(QvOSM_exUrl + "js/d3.v3.min.js", function() {
            Qva.LoadScript(QvOSM_exUrl + "js/ol.js", function() {
                ...
                //Definition.xml - type = select 인 텍스트가 작동하지 않을때,
                ...
                var drawMAP = function() {
                    ...
                    //console.log('pointPopup',pointPopup);
                    ...
                    pointPopupCloser.onclick = function() {
                        ...
                        if ($_VAR["vCntrShow_YN_P"] == "Y") {
                            Qv.GetCurrentDocument().SetVariable("vCntrShow_YN_P", "N");
                        }
                        ...
                        if ($_VAR["vMMSI_NO_P"] != "") {
                            Qv.GetCurrentDocument().SetVariable("vMMSI_NO_P", "");
                        }
                        if (focusOn)
                            drawTyphoonVessel();
                    ...
                    QvOSM_PVM_MAP.on("singleclick", function(evt) {
                        ...
                        var info = QvOSM_PVM_MAP.forEachFeatureAtPixel(evt.pixel, function(feature, layer) {
                            ...
                            port_cd = feature.get("tyKey") ? feature.get("tyKey") : feature.get("ref")["port_cd"];
                        ...
                        if (info != null && info.get("ref")) {
                            ...
                            switch ($obj["type"]) {
                                case "typhoon":
                                    if(vShow == "P") {
                                        Qv.GetCurrentDocument().SetVariable("vSEL_TYPHOON_P", $(this).attr("key_typhoon"));
                                        Qv.GetCurrentDocument().SetVariable("vSEL_TYPHOON_P2", $(this).attr("key_typhoon").slice(0,-5));
                                    } else if (vVIEWTYPE == "1") {
                            ...
                            if($obj == "port") {
                                Qv.GetCurrentDocument().SetVariable("vPORT_SELECT", info.get("text"));
                            }

                        } else {
                            if ($_VAR["vSEL_TYPHOON"])
                                if ($(".ol-selectable").css('display') != 'none') {
                                    drawTyphoonVessel();
                                    focusOnNum = "";
                                }
                        ...
                        if (info != null && popup) {
                            ...
                            if (focusOn) {
                                removeTyphoonVessel();
                                focusOnNum = info.O.ref.mmsi_no;
                                drawSelectTyphoonVessel(focusOnNum);
                            }
                            ...
                            if (('vessel' === (info.get('type') ? info.get('type') : info.get('ref')['type'])) || ('scheduled_dash_vessel' === info.get('ref')['type'])) {
                                $("#vLdContainers").on('click', function(e) {
                                    ...
                                    // }
                                    if(vShow == "P") {
                                        Qv.GetCurrentDocument().SetVariable("vCntrShow_YN_P", "Y");
                                        Qv.GetCurrentDocument().SetVariable("vMMSI_NO_P", singleClickTarget);
                                        Qv.GetCurrentDocument().SetVariable("vPORT", port_cd);
                                    } else {
                        ...
                        if(info === undefined && vPORT_SELECT !== undefined) {
                            vPORT_SELECT = undefined;
                            Qv.GetCurrentDocument().SetVariable("vPORT_SELECT", "");
                        }

                        $("#pointPopup").css("top", _top);
                        $("#pointPopup").css("left", _left);
                        //$("#pointPopup").css("height","250px");
                        $("#pointPopup").css("width", "300px");
                                ...
                                case "scheduled_dash_vessel":
                                    ...
                                    $.each(vTHRE_VESSEL_DATA_LOC[$obj.get("ref")['MMSI_NO']], function(key, value) {
                                        if (vTHRE_VESSEL_DATA_LOC[$obj.get("ref")['MMSI_NO']].length - 1 === key) {
                                            ...
                                            drawScheduledVesselRoute($obj.get("ref")['ROUT_CD'] + "_schedule_vessel_dash_" + $obj.get("ref")['MMSI_NO'], $obj.get("ref"), getroutepathCoord([value["START_POINT"], value["END_POINT"]]), QvOSM_PVM_MAP.getView().getZoom(), 'line', 'rgba(83, 211, 237, 0.5)', 3, 'mouseover');
                                        }
                                    })
                                    break;
                            }
                    ...
                    cqv.SetOnUpdateComplete(function() {
                        if (!varsRetrieved) {
                            ...
                            if (vTyphoon_Var_Lisr.Data.Rows.length > 0) {
                                ...
                                //23 vOnGoingTyphoonYN : 현재 태풍의 유무
                                if (vTyphoon_Var_Lisr.GetVariable(23).text) {
                                    //console.log(vTyphoon_Var_Lisr.GetVariable(23).text);
                                    try {
                                        vON_GOING_TYPHOON_YN = vTyphoon_Var_Lisr.GetVariable(23).text;
                                    } catch (e) {
                                        console.log('vTyphoon_Var_Lisr.GetVariable(21).text get Error', e);
                                    }
                                }

                                //24 vPORT_STATUS_JSON port와 관련된 vessel정보
                                if (vTyphoon_Var_Lisr.GetVariable(24).text) {
                                    //console.log(vTyphoon_Var_Lisr.GetVariable(24).text);
                                    try {
                                        if ('-' != vTyphoon_Var_Lisr.GetVariable(24).text && '' != vTyphoon_Var_Lisr.GetVariable(24).text && null != vTyphoon_Var_Lisr.GetVariable(24).text) {
                                            vPORT_STATUS_JSON = JSON.parse(vTyphoon_Var_Lisr.GetVariable(24).text);
                                        } else {
                                            vPORT_STATUS_JSON = [];
                                        }
                                    } catch (e) {
                                        console.log('vTyphoon_Var_Lisr.GetVariable(24).text get Error', e);
                                    }
                                }

                                //25 vPORT_JSON port정보
                                if (vTyphoon_Var_Lisr.GetVariable(25).text) {
                                    //console.log(vTyphoon_Var_Lisr.GetVariable(25).text);
                                    try {
                                        if ('-' != vTyphoon_Var_Lisr.GetVariable(25).text && '' != vTyphoon_Var_Lisr.GetVariable(25).text && null != vTyphoon_Var_Lisr.GetVariable(25).text) {
                                            vPORT_JSON = JSON.parse(vTyphoon_Var_Lisr.GetVariable(25).text);
                                        } else {
                                            vPORT_JSON = [];
                                        }
                                    } catch (e) {
                                        console.log('vTyphoon_Var_Lisr.GetVariable(25).text get Error', e);
                                    }
                                }

                                //26 vPORT_SELECT 
                                if (vTyphoon_Var_Lisr.GetVariable(26).text) {
                                    //console.log('vPORT_SELECT', vTyphoon_Var_Lisr.GetVariable(26).text);
                                    try {
                                        vPORT_SELECT = vTyphoon_Var_Lisr.GetVariable(26).text;
                                    } catch (e) {
                                        console.log('vTyphoon_Var_Lisr.GetVariable(25).text get Error', e);
                                    }
                                }
                                ...
                                //27 vShow 
                                if (vTyphoon_Var_Lisr.GetVariable(27).text) {
                                    //console.log('vShow', vTyphoon_Var_Lisr.GetVariable(27).text);
                                    try {
                                        vShow = vTyphoon_Var_Lisr.GetVariable(27).text;
                                    } catch (e) {
                                        console.log('vTyphoon_Var_Lisr.GetVariable(27).text get Error', e);
                                    }
                                } else {vPORT_SELECT == "";}
                            ...
                            cqv.GetAllVariables(function(variables) {
                                ...
                                try {
                                    ...
                                    try {
                                        ...
                                        if (vTY_STATUS_DATA)
                                            for (var i = 0; i < vTY_STATUS_DATA.length; i++) {
                                                ...
                                                vTY_STATUS_DATA[i]["popup"] = "MMSI : " + vTY_STATUS_DATA[i]["mmsi_no"] + "<br/>" //"Vessel Name : "+vTY_STATUS_DATA[i]["vsl_name"]+"<br/>"
                                                    +
                                    ...
                                    try {
                                        ...
                                        if (vPORT_STATUS_JSON) {
                                            for (var i = 0; i < vPORT_STATUS_JSON.length; i++) {
                                                // vTY_STATUS_DATA

                                                if (vPORT_STATUS_JSON[i]["typhoon_nm"] == "-") continue;
                                                if (vPORT_STATUS_JSON[i]["ltitde"] == "-" || vPORT_STATUS_JSON[i]["ltitde"] == "") continue;


                                                vPORT_STATUS_JSON[i]["lngtd"] = (vPORT_STATUS_JSON[i]["lngtd"] && $.isNumeric(vPORT_STATUS_JSON[i]["lngtd"])) ? parseFloat(vPORT_STATUS_JSON[i]["lngtd"]) : "-";
                                                vPORT_STATUS_JSON[i]["ltitde"] = (vPORT_STATUS_JSON[i]["ltitde"] && $.isNumeric(vPORT_STATUS_JSON[i]["ltitde"])) ? parseFloat(vPORT_STATUS_JSON[i]["ltitde"]) : "-";


                                                vPORT_STATUS_JSON[i]["point"] = [vPORT_STATUS_JSON[i]["lngtd"], vPORT_STATUS_JSON[i]["ltitde"]];
                                                //vPORT_STATUS_JSON[i]["direction"] = $.isNumeric(vPORT_STATUS_JSON[i]["direction"].replace("_", "")) ? vPORT_STATUS_JSON[i]["direction"].replace("_", "") : "00";
                                                vPORT_STATUS_JSON[i]["popup"] = "MMSI : " + vPORT_STATUS_JSON[i]["mmsi_no"] + "<br/>" //"Vessel Name : "+vPORT_STATUS_JSON[i]["vsl_name"]+"<br/>"
                                                    +
                                                    "Lat : " + vPORT_STATUS_JSON[i]["ltitde"] + "<br/>" +
                                                    "Lon : " + vPORT_STATUS_JSON[i]["lngtd"] + "<br/>" +
                                                    "Next Port : " + vPORT_STATUS_JSON[i]["nx_port_nm"] + "<br/>" +
                                                    (vPORT_STATUS_JSON[i]["typhoon_nm"] != "-" ? "Typhoon : " + vPORT_STATUS_JSON[i]["typhoon_nm"] + "(" + vPORT_STATUS_JSON[i]["typhoon_nm"] + ")" + "<br/>" : "") +
                                                    "Typhoon Time : " + vPORT_STATUS_JSON[i]["date"] +
                                                    "<p id='vLdContainers'>Loading Containers :<u>" + (vPORT_STATUS_JSON[i]["CNTR"] != "" ? vPORT_STATUS_JSON[i]["CNTR"] : "0") + "</u></p>";
                                                vPORT_STATUS.push(vPORT_STATUS_JSON[i]);
                                            }
                                            //console.log(vPORT_STATUS);
                                        }
                                    } catch (e) {
                                        console.log(e);
                                    }
                                    ...
                                    try {
                                        portDataArr = [];
                                        $.each(vPORT_JSON, function(indx, vPortJson) {
                                            let portLngtd = (vPortJson["lngtd"] && $.isNumeric(vPortJson["lngtd"])) ? parseFloat(vPortJson["lngtd"]) : "-"; //경도
                                            if (portLngtd == "-") return true;

                                            let portLtitde = (vPortJson["ltitde"] && $.isNumeric(vPortJson["ltitde"])) ? parseFloat(vPortJson["ltitde"]) : "-"; //위도
                                    ...
                                    //메인화면에서 현재 그릴 태풍이 없으면 마지막 태풍을 그린다 0805
                                    //if(vVIEWTYPE == "1" && vON_GOING_TYPHOON_YN == "N") vTYPHOON_DATA_ARR = vLAST_TYPHOON_ARR;
                                    ...
                                    var showVessel = function() {
                                        ...
                                        // 보라색 배 그리는 로직.
                                        /*if(vPORT_STATUS) {
                                            for(var ps_i=0; ps_i < vPORT_STATUS.length; ps_i++) {
                                                var truepoint = vPORT_STATUS[ps_i].point;
                                                var point = ol.proj.fromLonLat(vPORT_STATUS[ps_i].point);
                                                //console.log(vPORT_STATUS[vd_i].name+" : "+truepoint+" : "+point);

                                                var vIconFea = new ol.Feature(new ol.geom.Point(point));
                                                vIconFea.set('mmsi', vPORT_STATUS[ps_i].mmsi_no);
                                                vIconFea.set('tyKey', vPORT_STATUS[ps_i].vsl_name);
                                                vIconFea.set('popup', vPORT_STATUS[ps_i].popup);
                                                vIconFea.set('point1', truepoint);
                                                vIconFea.set('type', "vessel");
                                                //icon, rotation, scale, name, offx, offy


                                                vIconFea.set('style', createvesselIcon("normal", vPORT_STATUS[ps_i].icon, 0, 0.8));
                                                vIconFea.set('sel_style', createvesselIcon("zoom", vPORT_STATUS[ps_i].icon, 0, 0.8));
                                                vIconFea.set('zoom_style', createvesselIcon("zoom", vPORT_STATUS[ps_i].icon, 0, 0.8));
                                                vIconFea.set('namestyle', createvesselIcon("normal", vPORT_STATUS[ps_i].icon, 0, 0.8, vPORT_STATUS[ps_i].vsl_name, 0, offy));
                                                vIconFea.set('zoom_namestyle', createvesselIcon("zoom", vPORT_STATUS[ps_i].icon, 0, 0.8, vPORT_STATUS[ps_i].vsl_name, 0, offy))
                                                //console.log(vIconFea);
                                        ...
                                        $.each(vTYPHOON_DATA_ARR, function($k, $v) {
                                            ...
                                            if ($b4value != null) {
                                                $line_obj = {
                                                    key: $key,
                                                    data: [
                                                        [ol.proj.fromLonLat([$b4value["LNGTD"], $b4value["LTITDE"]]), ol.proj.fromLonLat([$v["LNGTD"], $v["LTITDE"]])]
                                                    ],
                                                    width: 3,
                                    ...
                                    var setStdRouteLine = function() {
                                        ...
                                        $.each(vSTD_DATA, function($key, $value) {
                                            ...
                                            if (vVIEWTYPE == 3 && route_no.substring(0, 1).toUpperCase() == "E") { //system auto
                                                $color = "rgba(255, 51, 153, 0.5)"; //분홍
                                            }
                                            ...
                                            ROUTE_LINE_ARR.push({
                                                ...
                                                data: [fn_pacificOceanRootModify($value["data"])], //태평양 처리 및 ol.proj.formLonLat 처리
                                                width: 1,
                                    ...
                                    var setSeaRouteLine = function() {
                                        ...
                                        $.each(vSEA_DATA, function($key, $value) {
                                            if ($b4value != null) {
                                                std_arr.push([
                                                    [$b4value["lngtd"], $b4value["ltitde"]],
                                                    [$value["lngtd"], $value["ltitde"]]
                                                ]);
                                                key = $value["key"];
                                            }
                                            $b4value = null;
                                    ...
                                    */
                                    ...
                                    if ($("#typhoon_div_wrapper_top").length == 0) {
                                        $("#typhoon_div").append('<div id="typhoon_div_wrapper_top" style="font-family:Calibri;font-size:18px;font-weight:bold;color:#ffffff;margin-left:-14px;">Typhoon &nbsp; &nbsp;<img style="width:18px; cursor:pointer; display:none;" id="refreshTyphoon" src="' + QvOSM_exUrl + 'icon/refresh_50.png"/></div><div id="typhoon_desc_wrapper" style="width:100%;height:390px;overflow-x:hidden;overflow-y:auto;"><div id="typhoon_desc" style="width:100%;"></div></div>');

                                        $("#refreshTyphoon").off("click").on("click", function(e) {
                                            focusOn = false;
                                            focusOnNum = "";
                                            $("#refreshTyphoon").hide();
                                            if ($_VAR["vSEL_TYPHOON"]) {
                                            ...
                                            if ($_VAR["vSEL_TYPHOON_P"]) {
                                                Qv.GetCurrentDocument().SetVariable("vSEL_TYPHOON_P", "");
                                                Qv.GetCurrentDocument().SetVariable("vSEL_TYPHOON_P2", "");
                                            }
                                        });
                                    }
                                    ...
                                    */
                                    ...
                                    $.each(vTYPHOON_DATA_ARR, function($key, $data) {
                                        ...
                                        //현재 태풍
                                        if ($data["IS_CURR"] == "Y" && vON_GOING_TYPHOON_YN == "Y") {
                                    ...
                                    if ($("#typhoon_desc").html() == "") {
                                        ...
                                        $.each(vLAST_TYPHOON_ARR, function($key, $data) {
                                            ...
                                            if ($key == vLAST_TYPHOON_ARR.length - 1) {
                                                //console.log('in');
                                                $("#typhoon_desc").append('<div style="color:#006ca2;font-size:25px;text-align:left;font-family:Calibri;font-weight:bold;"><br><br> &nbsp; There is no active Typhoon now.</div>');
                                    ...
                                    $("#typhoon_desc div#ty_title").off("click").on("click", function() {
                                        ...
                                        else Qv.GetCurrentDocument().SetVariable("vSEL_TYPHOON", $(this).attr("key_typhoon"));
                                    });
                                    ...
                                    var drawObj = function() {
                                        ...
                                        if(vShow_old != vShow) {
                                            $("#refreshTyphoon").trigger("click");
                                        }
                                        vShow_old = vShow;

                                        if (vVIEWTYPE == "1" && $_VAR["vSEL_TYPHOON"] && vShow == "T") {
                                            removeTyphoonVessel();
                                            drawTyphoonVessel();
                                        } else if(vPORT_SELECT !== undefined || vShow == "P") {
                                            if($_VAR["vSEL_TYPHOON_P"]) {
                                                $("#refreshTyphoon").show();
                                            }
                                            drawSelectPortVessel();
                                            drawPort();
                                        } else showVessel();
