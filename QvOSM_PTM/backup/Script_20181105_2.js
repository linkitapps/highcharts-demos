...
var drawScheduledVesselRoute = function( id, obj, route_arr, linecolor, lineWidth,  moseoverYn ){
...
var drawScheduledVesIcon = function( id, obj, point ){
    ...
    // var thr_sch_vesIconStyle=[];
    // thr_sch_vesIconStyle.push(
    //     new ol.style.Style({
    //         image: new ol.style.Icon({
    //             src: QvOSM_exUrl+'icon/'+'Vessel_'+obj["INOUT_IMG_NO"]+'.png',
    //             scale: 1
    //         }),
    //         text: new ol.style.Text({
    ...
    // );
    //
    // var thr_sch_vesIconZoomStyle=[];
    // thr_sch_vesIconZoomStyle.push(
    //     new ol.style.Style({
    //         image: new ol.style.Icon({
    //             src: QvOSM_exUrl+'icon/'+'Vessel_'+obj["INOUT_IMG_NO"]+'.png',
    //             scale: 1
    ...
    thr_sch_vesIconFeature.set( 'sel_style', createvesselIcon("zoom","Vessel_"+obj["INOUT_IMG_NO"], null, 0.8)  );
...
var drawScheduledVessel = function(){
    if( vTHRE_VESSEL_DATA_LOC    ){
                ...
                $.each( dataValue, function( $key, $value ){
                    ...
                    if( dataValue.length === 1 ){ // 좌표가 한개면 그냥 배 아이콘만 표시 되겠지? 점과 점이 연결되어야 선인데. 점 하나 밖에 없으니깐.
                        ...
                        if( $key === dataValue.length-1 ){
                            ...
                            drawScheduledVesselRoute($value['ROUT_CD']+"_schedule_vessel_dash_"+$value['MMSI_NO'], $value, getroutepathCoord( [$value["START_POINT"], dataValue[$key+1]["START_POINT"]]), 'rgb(79, 73, 73, 0.5)', 3, null );
                        }
                    }
                    // drawScheduledVesselRoute($value['ROUT_CD']+"_schedule_vessel_dash_"+$value['MMSI_NO'], $value, getroutepathCoord( [$value["START_POINT"], $value["END_POINT"]] ) );
                    drawScheduledVesIcon( $value['ROUT_CD']+"_schedule_vessel_dash_"+$value['MMSI_NO']+"_ICON", $value, $value["START_POINT"] );
                    // drawScheduledVesIcon( $value['ROUT_CD']+"_schedule_vessel_dash_"+$value['MMSI_NO']+"_ICON", $value, $value["END_POINT"] );
                    try{
                        // addLayer( $value['ROUT_CD']+"_schedule_vessel_dash_"+$value['MMSI_NO'] );
                    }catch(e){
                        console.log(e);
                    }
                });
        ...
        // $.each( vTHRE_VESSEL_LOC, function( $key, $value ){
        //     console.log( $value );
        //     $value['type'] = 'scheduled_dash_vessel';
        //     $value['vsl_name'] = $value["VSL_NAME"];
        //     $value['popup'] = "Vessel Name : "+$value["VSL_NAME"]+"<br/>"
        ...
        //         				+"Next Port : "+$value["ARRV_PORT_NM"];
        //
        //     if( vTHRE_VESSEL_LOC.length === 1 ){ // 좌표가 한개면 그냥 배 아이콘만 표시 되겠지? 점과 점이 연결되어야 선인데. 점 하나 밖에 없으니깐.
        //
        //     }else{ // 점이 1개 이상이닷? 그럼 점과 점 사이를 점선으로 표시하자.
        //         if( $key === vTHRE_VESSEL_LOC.length-1 ){
        //
        //         }else{
        //             $value['END_POINT'] = vTHRE_VESSEL_LOC[$key+1]["START_POINT"];
        //             drawScheduledVesselRoute($value['ROUT_CD']+"_schedule_vessel_dash_"+$value['MMSI_NO'], $value, getroutepathCoord( [$value["START_POINT"], vTHRE_VESSEL_LOC[$key+1]["START_POINT"]]), 'rgb(79, 73, 73, 0.5)', 3, null );
        //         }
        //     }
        //     // drawScheduledVesselRoute($value['ROUT_CD']+"_schedule_vessel_dash_"+$value['MMSI_NO'], $value, getroutepathCoord( [$value["START_POINT"], $value["END_POINT"]] ) );
        //     drawScheduledVesIcon( $value['ROUT_CD']+"_schedule_vessel_dash_"+$value['MMSI_NO']+"_ICON", $value, $value["START_POINT"] );
...
//							console.log($obj.get("ref")["tooltip"]);
                    ...
                    case "scheduled_dash_vessel" :
                        drawScheduledVesselRoute($obj.get("ref")['ROUT_CD']+"_schedule_vessel_dash_"+$obj.get("ref")['MMSI_NO'], $obj.get("ref"), getroutepathCoord( [$obj.get("ref")["START_POINT"], $obj.get("ref")["END_POINT"]] ), 'rgba(83, 211, 237, 0.8)', 5, 'mouseover' );
                        break;
					}
					...
					var showTyphoonIcon = function(){
						...
						if(TYPHOON_ICON_ARR.length>0){
							$.each(TYPHOON_ICON_ARR,function($key,$value){
                                ...
                                if($_VAR["vSEL_TYPHOON"]){
                                    $('#typhoon-descript')[0].innerText = "The Time of eash Typhoon and vessel's location is based on Korea Standard Time(GMT+9)";
                                    $('#typhoon-descript').css("top", ( window.screen.availHeight - 270 + ( undefined === window.scrollY ? 0 : window.scrollY ) ) + 'px')
                                }else{
                                    $('#typhoon-descript')[0].innerText = "";
                                }
...
*/
                    ...
                    if( $_VAR.vSEL_TYPHOON ){
                        ...
                        window.addEventListener('scroll', function(e) {
                            $('#typhoon-descript').css("top", ( window.screen.availHeight - 270 + $(this).scrollTop() ) + 'px')
                        });
                    }
