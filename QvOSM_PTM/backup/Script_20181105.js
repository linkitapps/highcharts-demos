...
var drawScheduledVesselRoute = function( id, obj, route_arr ){
...
var drawScheduledVesIcon = function( id, obj, point ){
    ...
    // thr_sch_vesIconFeature.set( 'tyKey', obj['VSL_NAME'] );
    // thr_sch_vesIconFeature.set( 'style',thr_sch_vesIconStyle  );
    // thr_sch_vesIconFeature.set( 'sel_style',thr_sch_vesIconStyle  );
    thr_sch_vesIconFeature.set( 'style', createvesselIcon("normal","Vessel_"+obj["INOUT_IMG_NO"], 0.8)  );
    thr_sch_vesIconFeature.set( 'sel_style', createvesselIcon("zoom","Vessel_"+obj["INOUT_IMG_NO"], 0.8)  );
        ...
        $.each( vTHRE_VESSEL_LOC, function( $key, $value ){
            ...
            if( vTHRE_VESSEL_LOC.length === 1 ){ // 좌표가 한개면 그냥 배 아이콘만 표시 되겠지? 점과 점이 연결되어야 선인데. 점 하나 밖에 없으니깐.
                ...
                if( $key === vTHRE_VESSEL_LOC.length-1 ){
                    ...
                    drawScheduledVesselRoute($value['ROUT_CD']+"_schedule_vessel_dash_"+$value['MMSI_NO'], $value, getroutepathCoord( [$value["START_POINT"], vTHRE_VESSEL_LOC[$key+1]["START_POINT"]] ) );
                }
            }
