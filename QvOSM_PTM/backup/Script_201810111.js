...
//				console.log(vCURR_TYPHOON);
				
				
				//maptype row data;
				if(vMAP==null) vMAP = cqv.GetObject("vMAP2");
				
				
				//현재화면 /히스토리 화면 전환용;
				oldVIEWTYPE = vVIEWTYPE;
				if(vCURR==null) vCURR = cqv.GetObject("vCURR");
				//현재 활동하는 태풍을 본다면 1 , 1: main 2: history 3:threshold
				try{
					vVIEWTYPE = ((vCURR.GetVariable()["text"]=="-" || vCURR.GetVariable()["text"]=="1") || vCURR.GetVariable()["text"]==null) ? "1" : vCURR.GetVariable()["text"];
...
//				console.log(vTYPHOON);
				...
				//태풍 데이터
				/*
				if(vTY_STATUS_DATA==null) vTY_STATUS_DATA = cqv.GetObject("vTY_STATUS");
				try{
					vTY_STATUS_DATA.Data.SetPagesizeY(vTY_STATUS_DATA.Data.TotalSize.y);
				}catch(e){
					IsChkCnt++;
					console.log(e);
				}
				console.log(vTY_STATUS_DATA);
				*/
...
//					console.log("vTO_PORT :: "+vTO_PORT);
				...
				//현재 태풍이 없으면 제일 마지막으로 발생한 태풍을 가져와서 보여준다
				if(vLAST_TYPHOON==null) vLAST_TYPHOON = cqv.GetObject("vLAST_TYP");
				
				//threshold 화면에서 표준 route data
...
//						console.log("vMAPTYPE :: "+vMAPTYPE);
					...
					try{
						vSTD_YN = vSTD_ROUTE_YN.GetVariable()["text"];
//						console.log("vSTD_YN :: "+vSTD_YN);
					...
					try{
						vTHRE_YN = vSTD_THRE_YN.GetVariable()["text"];
...
//						console.log("vSEL :: "+vSEL);
					...
					vLAST_TYPHOON_NM;;
					try{
						vLAST_TYPHOON_NM = vLAST_TYPHOON.GetVariable()["text"];
//						console.log("vLAST_TYPHOON_NM :: "+vLAST_TYPHOON_NM);
					...
					try{
						...
						$.each(JSON.parse(vTY_STATUS_DATA.Data.Rows[0][2]["text"]),function($k,obj){
							...
							var obj = {
									...
									typhoon_nm		:	this[1]["text"],
									mmsi_no			:	this[2]["text"],
									status			:	this[3]["text"],
									date			:	this[4]["text"],
									lngtd			:	(this[5] && $.isNumeric(this[5]["text"]))	?	parseFloat(this[5]["text"])	:	"-",//경도
									ltitde			:	(this[6] && $.isNumeric(this[6]["text"]))	?	parseFloat(this[6]["text"])	:	"-",//위도
									vsl_name		:	this[7]["text"],
									direction		:	this[8]["text"],
									b4_port_cd		:	this[9]["text"],
									b4_port_nm		:	this[10]["text"],
									nx_port_cd		:	this[11]["text"],
									nx_port_nm		:	this[12]["text"],
									icon			:	this[13]["text"],
									popup			:	null
							}
							*/
							
							//if(obj["status"]=="G") return;
							//if(obj["status"]=="Z") return;
							if(obj["key_typhoon"]=="-") return;
							if(obj["ltitde"]=="-" || obj["ltitde"]=="-") return;
							
							
							obj["lngtd"] = (obj["lngtd"] && $.isNumeric(obj["lngtd"]))	?	parseFloat(obj["lngtd"])	:	"-";
							obj["ltitde"] = (obj["ltitde"] && $.isNumeric(obj["ltitde"]))	?	parseFloat(obj["ltitde"])	:	"-";
							
							
							obj["point"] = [obj["lngtd"],obj["ltitde"]];
							obj["direction"] = $.isNumeric(obj["direction"].replace("_","")) ? obj["direction"].replace("_","") : "00";
							obj["popup"] = "Vessel Name : "+obj["vsl_name"]+"<br/>"
											+"MMSI : "+obj["mmsi_no"]+"<br/>"
											+"Lat : "+obj["ltitde"]+"<br/>"
											+"Lon : "+obj["lngtd"]+"<br/>"
											+"Next Port : "+obj["nx_port_nm"]+"<br/>"
											+(obj["typhoon_nm"]!="-" ? "Typhoon : "+obj["typhoon_nm"]+"("+obj["typhoon_nm"]+")"+"<br/>" : "")
											+"Typhoon Time : "+obj["date"];
							
							switch(obj["status"]){
							case "R": //red
								obj["icon"] = "Vessel_EXCEPTION"+(obj["direction"]!="" ? "_"+obj["direction"] : "00");
								break;
							case "Y": //yellow
								obj["icon"] = "Vessel_NOTMOVING"+(obj["direction"]!="" ? "_"+obj["direction"] : "00");
								break;
							case "G": //yellow
								obj["icon"] = "Vessel_MOVING"+(obj["direction"]!="" ? "_"+obj["direction"] : "00");
								break;
							case "Z": //gray
								obj["icon"] = "Vessel_Gray"+(obj["direction"]!="" ? "_"+obj["direction"] : "00");
								break;
							};
							
							
							if(vTY_STATUS[obj["key_typhoon"]]==null){
								vTY_STATUS[obj["key_typhoon"]] = {};
							}
							
							if(vTY_STATUS[obj["key_typhoon"]][obj["mmsi_no"]]==null){
								vTY_STATUS[obj["key_typhoon"]][obj["mmsi_no"]] = new Array();
							}
							
							vTY_STATUS[obj["key_typhoon"]][obj["mmsi_no"]].push(obj);
						});
					}catch(e){
						console.log(e);
					}
					...
					try{
						...
						$.each(vTYPHOON.Data.Rows,function(){
							...
							//vVIEWTYPE == "1" 일태 적어도 하나의 태풍은 보여줘야 한다. 태풍정보에 담았다가 
...
//						console.log(vHIS_DATA);
					...
					var setRouteLine = function(){
						...
						try{
							vTHRESHOLD = vTHRE.GetVariable(0)["text"];
							vTHRE_CNT = vTHRE.GetVariable(1)["text"];
//							console.log("vTHRESHOLD :: "+vTHRESHOLD);
//							console.log("vTHRE_CNT :: "+vTHRE_CNT);
						...
						try{
							vNEWTHRESHOLD = vNTHRE.Data.Rows.length>0 ? vNTHRE.GetVariable()["text"] : "N/A";
