...
var routeDraw = function() {
    ...
    */
    ...
    var aniCoord2 = [];
    var arcCoordstyle2 = [];
    var arcCoordafterstyle2 = [];
    var aniCoordsLength2;
    //test
    var testCenter = [];
    ...
    for (i = 0; i < vPATH_VALUELength; i++) {
        ...
        if (i < vPATH_VALUELength - 1) {
            ...
            if (vPATH_VALUE[i].ROUTE == "") {
                ...
                if (vPATH_VALUE[i].ROUTE_EXP) {
                    arcCoordafter2 = getroutepathCoord(vPATH_VALUE[i].ROUTE_EXP);

                    arcCoord2 = arcCoord2.concat(arcCoordafter2);
                }
            ...
            if (arcCoord.length > 1) {
                ...
                if (arcCoord2.length > 1) {
                arcCoordLength2 = arcCoord2.length - 1;

                //4.운송수단 feature 만들기

                //애니메이션 좌표는 따로 저장
                if (vPATH_VALUE[i]["STATUS"] === "D" && vPATH_VALUE[i + 1]["STATUS"] === "N") {
                    aniCoord2 = arcCoord2.slice(0);

                    //마지막 조금 남아있다 처음으로 돌아가기
                    for (var ai = 0; ai < 30; ai++) {
                        aniCoord2.push(aniCoord2[arcCoordLength2]);
                    }

                } else {}

                //멀티라인 좌표로 만들기
                for (j = 0; j < arcCoordLength2; j++) {
                    //                  /* 곡선 표현시 해당 로직추가
                    ...
                    //}
                    var lineCoord = [];
                    lineCoord.push([arcCoord2[j], arcCoord2[j + 1]]);
        ...
        //console.log(point1);
        QvOSM_PVM_MAP.addOverlay(makerouteLabel(vPATH_VALUE[i]["DISP_DATE"], point1));
