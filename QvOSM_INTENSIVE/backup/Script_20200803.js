...
let showVesselIcon = function() {
    let vIconFeas = [];
    vesselDataArr.forEach(function(vesselData) {
        console.log('vesselData',vesselData);
        let truepoint = vesselData.point;
        let point = ol.proj.fromLonLat(vesselData.point);
        let vIconFea = new ol.Feature(new ol.geom.Point(point));
        vIconFea.set('mmsi', vesselData.mmsi);
        vIconFea.set('tyKey', vesselData.name);
        vIconFea.set('popup', vesselData.popup);
        vIconFea.set('point1', truepoint);
        vIconFea.set('type', "vessel");
        vIconFea.set('style', createVesselIcon("normal", "Vessel_" + vesselData.inout, vesselData.direction, 0.8));
        vIconFea.set('sel_style', createVesselIcon("zoom", "Vessel_" + vesselData.inout + "", vesselData.direction, 0.8));
        vIconFea.set('zoom_style', createVesselIcon("zoom", "Vessel_" + vesselData.inout + "", vesselData.direction, 0.8));
        vIconFea.set('namestyle', createVesselIcon("normal", "Vessel_" + vesselData.inout, vesselData.direction, 0.8, vesselData.name, 0, offy));
        vIconFea.set('zoom_namestyle', createVesselIcon("zoom", "Vessel_" + vesselData.inout, vesselData.direction, 0.8, vesselData.name, 0, offy));

        offy = (offy === 15) ? 22 : 15;
        if (vesselData.aniYN === 'Y') {
            vIconFea.set('key', vesselData.name);
    ...
    console.log('vesselvIconFeas',vIconFeas);
