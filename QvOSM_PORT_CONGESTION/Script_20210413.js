...
var setRiskScore = function loaddata(){
	...
	if($target.Data.Rows!=null && $target.Data.Rows.length>0 && $target.Data.Rows[0].length==12)
	$.each($target.Data.Rows,function($k,$v){
		var obj = {
				...
				port_natn	:	$v[6]["text"],
				port_aging  :   ($.isNumeric($v[7]["text"]) ? parseFloat($v[7]["text"]) : ""),
				sum_anch    :   ($.isNumeric($v[8]["text"]) ? parseFloat($v[8]["text"]) : ""),
				sum_port    :   ($.isNumeric($v[9]["text"]) ? parseFloat($v[9]["text"]) : ""),
				// vessels     :   parseFloat($v[11]["text"]),
				vessels     :  $v[10]["text"],
				port_aging_h:   Math.round($v[11]["text"]),
		 ...
		 //console.log($v[0]["text"],$v[1]["text"],$v[2]["text"],$v[3]["text"],$v[4]["text"],$v[5]["text"],$v[6]["text"],$v[7]["text"],$v[8]["text"],$v[9]["text"],$v[11]["text"])
		/*var obj = {
				type		:	"score",
				date		:	$v[0]["text"],
				loc_cd		:	$v[1]["text"],
		...
		obj["tooltip"] = "<div style='font-size:12px;font-weight:bold;'>"+obj["port_cd"] + " (" + obj["port_nm"] + ", " + obj["port_natn"] + ")" + "</div>"
		+"<div style='font-size:12px;'>Container Ships ("+obj["vessels"] + " vessels)"+"</div>"
		+"<div style='font-size:12px;'>"+obj["port_aging"]  +" days ("+  obj["port_aging_h"] +" hrs)" + "</div>"
		+"<div style='font-size:12px;'> at Anchorage: "+obj["sum_anch"] + " d (std " + obj["anch_stdev"] + " d)" + "</div>"
		+"<div style='font-size:12px;'> at Port: "+obj["sum_port"] + " d (std " + obj["port_stdev"] + " d)" + "</div>";



		//김민근 원본 주석후 아래에 port_aging으로 조건문 걸어놓음
		...
		var iconFeature = new ol.Feature({
			...
			port_aging  :   obj["port_aging"],	//7
			sum_anch    :  	obj["sum_anch"],	//8
			sum_port    :   obj["sum_port"],	//9
			vessels     :   obj["vessels"],     //10

			port_aging_h:   obj["port_aging_h"], //11
			//
			color		:	obj["color"],
			width		:	obj["width"],
			icon		:	obj["icon"],
	...
	QvOSM_PVM_MAP.on("wheel", function(e) {
		var zoomlevel = QvOSM_PVM_MAP.getView().getZoom();
...
/* 	vSTD_DATA = {};
	$.each(vSTD,function($k,$v){

		if($v["ROUT_CD"]=="-" || $v["ROUT_CD"]=="") return true;
        // console.log($v);
        // console.log( $v.STD_LNGTD_LTITDE );
        // console.log( JSON.parse($v["STD_LNGTD_LTITDE"]) );
        if( "" != $v.STD_LNGTD_LTITDE ){
            vSTD_DATA[$v["ROUT_CD"]] = JSON.parse($v["STD_LNGTD_LTITDE"]);
        }else{
            vSTD_DATA[$v["ROUT_CD"]] = "";
        }
	...
	vSEA_DATA = {};
	$.each(vSEA,function($k,$v){

		if($v["ROUT_CD"]=="-" || $v["ROUT_CD"]=="") return true;

        if( "" != $v["SEARATE_LNGTD_LTITDE"] ){
            vSEA_DATA[$v["SEARATE_LNGTD_LTITDE"]] = JSON.parse($v["SEARATE_LNGTD_LTITDE"]);
        }else{
            vSEA_DATA[$v["SEARATE_LNGTD_LTITDE"]] = "";
        }
