...
function showgridpopup(event,$curr_data){
   ...
   $("#detail_pointPopup .ol-popup-title").attr("type",$curr_data["type"]);
	...
	if($curr_data["type"]=="news"){ //news popup 최초 그리는 부분
		...
		$("#detail_pointPopup #popup-content .container_cnt").html(($curr_data["container_cnt"] ? $curr_data["container_cnt"]+" container(s)" : ""));
					...
					if(info!=null && (info.get("news_arr")!=null || info.get("except_arr")!=null) && (info.get("news_arr").length>0 || info.get("except_arr").length>0)){
						...
						function btn_next($data){
							...
							if($type=="news"){ //news popup 최초 그리는 부분
								...
								$("#pointPopup #popup-content .container_cnt").html(($curr_data["container_cnt"] ? $curr_data["container_cnt"]+" container(s)" : ""));
                        ...
                        console.log( info.get("type") );
                        console.log( info.get("tyKey") );
                        console.log( info.get("popup") );
                        // $(".ol-popup-headtitle-content").html("");
                        // debugger;
						$(".ol-popup-headtitle-content").text(info.get("tyKey"));
                        // $(".ol-popup-title-content").hide();
                        // $(".ol-popup-title-content").text("X");
				...
				$('#ticker_cont').off("mouseover").on("mouseover",function(evt){
					...
					function btn_next($data){
						...
						if($type=="news"){ //news popup 최초 그리는 부분
							...
							$("#ticker_pointPopup #popup-content .container_cnt").html(($curr_data["container_cnt"] ? $curr_data["container_cnt"]+" container(s)" : ""));
