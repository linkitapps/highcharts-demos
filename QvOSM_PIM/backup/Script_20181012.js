					...
					try {
						...
						var setTyphoonIcon = function(){
							...
							var destinationPoint = function(lon, lat, distance, bearing, radius) {
								...
								// vPai2 = asin( sinvPai1⋅cosvDelta + cosvPai1⋅sinvDelta⋅cosvTheta )
								// vLambda2 = vLambda1 + atan2( sinvTheta⋅sinvDelta⋅cosvPai1, cosvDelta − sinvPai1⋅sinvPai2 )
								// see http://williams.best.vwh.net/avform.htm#LL

								var vDelta = Number(distance) / radius; // angular distance in radians
								var vTheta = Number(bearing) * Math.PI / 180;

								var vPai1 = lat.toRadians();
								var vLambda1 = lon.toRadians();

								var vPai2 = Math.asin(Math.sin(vPai1)*Math.cos(vDelta) + Math.cos(vPai1)*Math.sin(vDelta)*Math.cos(vTheta));
								var x = Math.cos(vDelta) - Math.sin(vPai1) * Math.sin(vPai2);
								var y = Math.sin(vTheta) * Math.sin(vDelta) * Math.cos(vPai1);
								var vLambda2 = vLambda1 + Math.atan2(y, x);

								//[lon, lat]
								return [(vLambda2.toDegrees()+540)%360-180, vPai2.toDegrees()]; // normalise to −180..+180°
							};
