...
//							url: 'https://api.mapbox.com/styles/v1/seungok/ciov2x7gj003qdpnjymn2em34/tiles/{z}/{x}/{y}?access_token=pk.eyJ1Ijoic2V1bmdvayIsImEiOiJjaW5oN3A4dWYwc2dxdHRtM2pzdDdqbGtvIn0.JLtJmHeZNzC5gg_4Z6ioZg'
			...
			var drawINCIDENT = function(){
                ...
                // var SOURCE_OBJ = {};
                if( vPolygonArr.length ){
                    var vPolygonFeas = [];

                    polygonLayer = QvOSM_PVM_MAP.removeLayer(polygonLayer);
                    for( var i = 0; i < vPolygonArr.length ; i++ ){
                        // console.log( vPolygonArr[i] );
                        var geoArr = vPolygonArr[i].pointer;
                        for( var j = 0 ; j < geoArr.length ; j++ ){
                            // console.log( geoArr[j] );
                            geoArr[j] = ol.proj.fromLonLat(geoArr[j]);
                            // console.log( geoArr[j] );
                        }
                        console.log(geoArr );
                        var geojsonObject2 = {
                        ...
                        geoStyle_obj = new ol.geom.Polygon(geoArr);
                        // console.log( geoStyle_obj );
                        var feature = { 'type': 'Feature',
                        				'geometry': {
                    						'type': 'Polygon',
                    						'coordinates': geoArr
                        ...
                        style_obj = new ol.style.Style({
                    				    stroke: new ol.style.Stroke({
                    					color: 'rgba( 239,85,41, 1 )',
                    					width: 1
                    				}),
                    				fill: new ol.style.Fill({
                    					color: 'rgba( 239,85,41,0.8 )'
                    				})
                    			});
                        // var vPolygonFeas = [];
                        // var vPolygonIconVector = new ol.source.Vector({
    					// 					features: geoFeatures2
    					// 				});

                        var lLines2 = [];
                        if( SOURCE_OBJ["polygon"] == null ){
                            SOURCE_OBJ["polygon"] = new ol.source.Vector({
                                features: geoFeatures2
                            });
                        }
                        ...
                        var lFea2 = new ol.Feature({
                    			  geometry: geoStyle_obj,
                    			  name: vPolygonArr[i]["key"]
                    		  });

                        style_obj.setZIndex(1000);
                        lFea2.set("ref",vPolygonArr[i]);
                    	lFea2.set("style", style_obj);

                        lLines2.push(lFea2);
                    	SOURCE_OBJ["polygon"].addFeatures(lLines2);

                        // var vPolygonIconFea2 = new ol.Feature({
                		// 	  geometry: geoStyle_obj,
                		// 	  name: vPolygonArr[i]["key"]
                		//   });
                        // vPolygonIconFea2.set( 'style', style_obj );
                        // vPolygonFeas.push( vPolygonIconFea2 );
                        // // vPolygonIconVector.addFeatures( vPolygonFeas );
                        //
                        // // style_obj.setZIndex(1300);
                        if( LAYER_OBJ["polygon"] == null ){
                            LAYER_OBJ["polygon"] = new ol.layer.Vector({
                    			source: SOURCE_OBJ[vPolygonArr[i]["key"]],
                    			style: function(feature){
                    				return feature.get("style");
                    			}
                    		});
                        }else{
                            QvOSM_PVM_MAP.removeLayer(LAYER_OBJ["polygon"]);
                        }

                        try{
                    		QvOSM_PVM_MAP.addLayer(LAYER_OBJ["polygon"]);
                    	}catch(e){
                    		console.log(e);
                    	}
                    }//for end
					...
					// 							return feature.get('style');
					// 						},
					// 				source:vPolygonIconVector
					// 			});
					// polygonLayer.setZIndex(1000);
                    // console.log( vPolygonIconVector );
			...
			Qva.AddExtension(ExName, function() {
				...
				if(!varsRetrieved){
                    ...
                    if( vEXT_INCIDENT.Data.Rows.length ){
                        ...
                        // GetVariable(10) : vPOLYGON 폴리곤(다각형) 데이터
...
//								console.log(list);
						...
						$.each(vNewsObj,function($key,$value){
                            ...
                            var container_cnt	= ($value["NEWS_CNTR_CNT"] && $.isNumeric($value[15]))	        ?	$value["NEWS_CNTR_CNT"]	            :	"";//container_cnt
                        ...
                        var vPOLYGON_length = vPOLYGON.length
                        if( vPOLYGON_length > 0 ){
                            vPolygonArr = [];
                            for( var i = 0 ; i < vPOLYGON_length ; i++ ){
                                var polyObj = {};
                                polyObj.pointer = JSON.parse( vPOLYGON[i].INCIDENT_POLYGON_LNGTD_LTITDE ),
                                polyObj.id = vPOLYGON[i].INCID_ID,
                                polyObj.name = vPOLYGON[i].POLYGON_NAME
                                vPolygonArr.push( polyObj );
                            }
                            console.log( vPolygonArr );
                        }
