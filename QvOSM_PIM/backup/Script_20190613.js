	...
	Qva.LoadScript(QvOSM_exUrl+"js/d3.v3.min.js", function(){
		Qva.LoadScript(QvOSM_exUrl+"js/ol.js", function(){
			...
			var drawMAP = function(){
				...
				var htmlString = "";
				htmlString += '<div id="pointPopup" class="ol-popup" idx="-1" style="display:none;"><div class="ol-popup-title"><div class="ol-popup-headtitle-content" style="color:#FFFFFF;"></div><div class="ol-popup-title-content" style="float:right;color:#FFFFFF;"><button id="exception_button" class="btn btn-default">exc</button><span id="btn_prev">◀</span><font id="disp_step" style="margin-left:2px;margin-right:2px;font-weight:normal;"></font><span id="btn_next">▶</span></div></div><a href="#" id="popup-closer" class="ol-popup-closer" style="cursor:pointer;display:none;"></a><div id="popup-content"></div></div><div id="geo-marker"></div>'+
						...
						function btn_next($data){
							...
							if($type=="news"){ //news popup 최초 그리는 부분
								...
								if("Y" == $curr_data["anyFlag"]) {
									$("#exception_button").show();
									$("#exception_button").off('click').on('click', function(evt) {
										$("#pointPopup").hide();
										$("#ticker_pointPopup").hide();
										exc_btn_bool = true;
										exc_btn_id = $curr_data["id"];
										Qv.GetCurrentDocument().SetVariable("vRELATION_FLAG",$curr_data["id"]);
										console.log("=====setvariable===================================");
										console.log($curr_data["id"]);
									});
								} else {
									$("#exception_button").hide();
								}
