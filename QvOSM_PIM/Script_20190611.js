				...
				if(incidentData.length>0){
					...
					$.each(incidentData,function($key,incidentObj){
						...
						if(incidentObj["news_arr"].length>0 || incidentObj["except_arr"].length>0){
							if( "Y" == incidentObj["news_arr"].anyFlag ) {
				...
				$.each(incidentData,function($key,incidentObj){
					//console.log('incidentObj: ', incidentObj);
						if( incidentObj["news_arr"].length>0 ) {
							//console.log('tick : ', tick["news_arr"][0].id);
							//console.log('incidentObj : ', incidentObj["news_arr"][0].id);
							for(let news of incidentObj["news_arr"]) {
								if(tick["news_arr"][0].id == news.id) {
									vIconFea = drawINCIDENT_icon(incidentObj);
									break;
								}
							}
					...
					/*console.log('vIconFea: ', vIconFea);
				if( "" == vIconFea ) {
					console.log("들어옴");
					$.each(incidentData,function($key,incidentObj){
						if(incidentObj["loc_nm"] == tick["loc_nm"]) {
							vIconFea = drawINCIDENT_icon(incidentObj);
							return false;
						}
					});
...
*/
				...
				incidentLayer.setZIndex(500);
				QvOSM_PVM_MAP.addLayer(incidentLayer);

				
			};
			
			var drawINCIDENT_tickerMouseout = function(){
				//zIndex = 0;

				incidentLayer = QvOSM_PVM_MAP.removeLayer(incidentLayer);
//				console.log(incidentLayer);
				if(incidentData.length>0){
					...
					//console.log('vIconVector: ', vIconVector);
			...
			Qva.AddExtension(ExName, function() {
				...
				if(!varsRetrieved){
					...
					//console.log('vExceptObj: ', vExceptObj);
