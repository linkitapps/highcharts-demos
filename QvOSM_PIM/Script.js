...
var vToolTip_YN;  // tooltip on/off
var currentZoomLevel;	//20190611
var lockBoolean=false;	//20190611
var oldIncidentLayer;
var exc_btn_bool=false;
var exc_btn_id;
var alarm_bool=false;

var incidentIconAni = [];
var incidentIconAniobj = [];
var hotExcIconAni = [];
var hotExcIconAniobj = [];
var IconAni = [];
var IconAniobj = [];
var alwaysTooltipOverlays = [];
...
var ticker_tooltipPop;
$(".TextObject").off("mouseover").on("mouseover", function(){
	exc_btn_bool=false;
});
...
Qva.LoadScript(QvOSM_exUrl+"js/jquery.marquee.js", function(){
	Qva.LoadScript(QvOSM_exUrl+"js/d3.v3.min.js", function(){
		Qva.LoadScript(QvOSM_exUrl+"js/ol.js", function(){
			...
			function fn_aniFlash( feature,number,type ){
				...
				function animate(incident) {
					//console.log( 'sss : ', incidentIconAni );
					var vectorContext = incident.vectorContext;
				    var frameState = incident.frameState;
					var flashGeom = feature.getGeometry().clone();
					var elapsed = frameState.time - start;
					var elapsedRatio = elapsed / duration;
					// radius will be 5 at start and 30 at end.
					var radius = ol.easing.easeOut(elapsedRatio) * 10 + 16;
					var opacity = ol.easing.easeOut(1 - elapsedRatio);
					var style
					if(true){
						style = new ol.style.Style({
				...
				listenerKey = QvOSM_PVM_MAP.on('postcompose', animate);
				if('news' == type && !incidentIconAni[number]){
					//console.log('new num : ',number);
					incidentIconAni[number]=setInterval(function(){fn_aniFlash(feature,number, 'news')},9000);
				} else if('except' == type && !hotExcIconAni[number]){
					hotExcIconAni[number]=setInterval(function(){fn_aniFlash(feature,number, 'except')},9000);
				} else if('incident' == type && !IconAni[number]){
					IconAni[number]=setInterval(function(){fn_aniFlash(feature,number, 'incident')},9000);
				}
			}
			...
			var drawMAP = function(){
				...
				htmlString += '<div id="ticker_tooltip"></div>';
				htmlString += '<div id="div_alwaysTooltips"></div>';
				...
				var ticker_pointPopupOnMap = '<div id="ticker_pointPopupOnMap" class="ol-popup" style="display:none;"><div class="ol-popup-title"><div class="ol-popup-headtitle-content" style="color:#FFFFFF;"></div><div class="ol-popup-title-content" style="float:right;color:#FFFFFF;"></div></div><a href="#" id="popup-closer" class="ol-popup-closer" style="cursor:pointer;display:none;"></a><div id="popup-content"></div></div>';

				$("#ticker_pointPopupOnMap").remove();
				$("body").append(ticker_pointPopupOnMap);
...
//							url: 'https://api.mapbox.com/styles/v1/seungok/ciov2x7gj003qdpnjymn2em34/tiles/{z}/{x}/{y}?access_token=pk.eyJ1Ijoic2V1bmdvayIsImEiOiJjaW5oN3A4dWYwc2dxdHRtM2pzdDdqbGtvIn0.JLtJmHeZNzC5gg_4Z6ioZg'
				...
				QvOSM_PVM_MAP.on("singleclick", function (evt) {
					...
					if(info!=null && (info.get("news_arr")!=null || info.get("except_arr")!=null) && (info.get("news_arr").length>0 || info.get("except_arr").length>0)){
						...
						function btn_next($data){
							...
							if($type=="news"){ //news popup 최초 그리는 부분
								...
								//console.log( 'vdt :', vdt );
								if( "Y"== $curr_data["anyFlag"] ){
								$("#pointPopup #popup-content .div_newsPop_img").html("<a target='_blank' href='"+QvOSM_exUrl+"imageHtml.html&qImageUrl="+$curr_data["imgUrl"]+"'><img class='img_newsAddress' src='"+$curr_data["imgUrl"]+"' style='width:57%; float:right;'></a>");
								}else{
									//$("#pointPopup #popup-content .div_newsPop_img").html("");
								}
								...
								$("#pointPopup #popup-content .container_cnt").css("cursor","pointer").css("text-decoration","underline").off("click").on("click",function(){
									...
									if("" == $curr_data["newsVessel"]) {
										Qv.GetCurrentDocument().SetVariable("vID", $curr_data["id"]);
										Qv.GetCurrentDocument().SetVariable("vLOC_CD", $curr_data["loc_cd"]);
									} else {
										console.log('setval', $curr_data["newsVessel"]);
										Qv.GetCurrentDocument().SetVariable("vNEWS_VESSEL", $curr_data["newsVessel"]);
									}
									console.log("=====qlikview popup show===================================");
								});
					...
					// 20190613추가 선박사고 exception
					if(info!=null && info.get("news_arr")!=null && info.get("news_arr").length>0 ){
						for(var i=0, news; news=info.get("news_arr")[i]; i++) {
							if("Y" == news.anyFlag) {
								exc_btn_bool = true;
								exc_btn_id = news.id;
								Qv.GetCurrentDocument().SetVariable("vRELATION_FLAG",news.id);
				...
				QvOSM_PVM_MAP.on('pointermove', function(evt) {
					...
					var ret = false;
					  var feature = QvOSM_PVM_MAP.forEachFeatureAtPixel(evt.pixel, function(feature) {
						    ...
						    if(feature.get("tooltip")!=null && feature.get("type") != 'vessel' && 'ON' != vToolTip_YN){
						    ...
						    if(feature.get("popup")!=null && feature.get("type") != 'vessel' && 'ON' != vToolTip_YN ){
			...
			var drawINCIDENT_icon = function(incidentObj) {
				...
				if( 'ON' == vToolTip_YN ){
					var tooltipHtml = '';
					if( 'news' == incidentObj["type"] ){
						tooltipHtml = '<div class="div_tooltip_el tooltipType_news" id="tooltip_'+incidentObj["loc_nm"]+'">'+tooltip+'</div>';
					}else{
						tooltipHtml = '<div class="div_tooltip_el tooltipType_except" id="tooltip_'+incidentObj["loc_nm"]+'">'+tooltip+'</div>';
					}

					$("#div_alwaysTooltips").html( tooltipHtml );
					//tooltip
					var tooltipEl = document.getElementById( 'tooltip_'+ incidentObj["loc_nm"]);
					var tooltipPos = ol.proj.fromLonLat( incidentObj["pos"] );
					var tooltipPop = new ol.Overlay({
						element: tooltipEl,
						position: tooltipPos,
						positioning: 'bottom-left',
						offset: [20,20]
					...
					QvOSM_PVM_MAP.addOverlay(tooltipPop);
					alwaysTooltipOverlays.push( tooltipPop );
				}
				
				return vIconFea;
			}

			var drawIncidentLayer = function(vIconFeas) {
				
				if(incidentData.length>0){
					if(undefined === vIconFeas && !lockBoolean) {
						var vIconFeas = [];
						//console.log(incidentData.length);
						var yAni_incidentData = [];
						//var incidentDetail = JSON.parse(docObj.GetVariable(0).text);
						
						$.each(incidentData,function($key,incidentObj){
							/*if( incidentObj['type'] != 'news' ){
								console.log( incidentObj );
							}*/
							if(incidentObj["news_arr"].length>0) {
								var drawIconBool = false;
								for(var i=0, news; news=incidentObj["news_arr"][i]; i++) {
									if( (4 > QvOSM_PVM_MAP.getView().getZoom() && "Y"==news.newsMinLocYn) || 4 <= QvOSM_PVM_MAP.getView().getZoom()) {
										if( "Y" == news.anyFlag ) {
										yAni_incidentData.push(incidentObj);
										drawIconBool = false;
										break;
									}
										drawIconBool = true;
						...
						$.each(yAni_incidentData,function($key,incidentObj) {
							var vIconFea = drawINCIDENT_icon(incidentObj);
							//console.log(vIconFea);
						...
						drawPolygonLayer();
					} else if(undefined === vIconFeas && lockBoolean){ 
						exc_btn_bool = false;
						QvOSM_PVM_MAP.removeLayer(incidentLayer);
						incidentLayer = oldIncidentLayer;
						QvOSM_PVM_MAP.addLayer(incidentLayer);
						return;
					}
					oldIncidentLayer = QvOSM_PVM_MAP.removeLayer(incidentLayer);
			...
			var drawINCIDENT = function(){
				...
				$.ticker_next = function(){
					if(ticker_incidentData.length > 0) {
					var ticker_step = $("#ticker_cont").attr("ticker_step")==-1 ? -1 : parseInt($("#ticker_cont").attr("ticker_step"));
					ticker_step++;

					try{
						//console.log("ticker_step",ticker_step);
						//console.log(ticker_incidentData.length);
						if(ticker_step >= ticker_incidentData.length){
							ticker_step = 0;
						}
						...
						case "except":
							...
							var level = incidentObj["level"];
							if(title.length>100){
								title = title.substr(0,100) + "...";
							}
							$("#ticker_cont").html("[Exception"+level+"] "+title+" ("+ocdt+")");
							break;
						}
						...
						//tooltip 노출
						/*if("-" != incidentObj["pos"][0] && "-" != incidentObj["pos"][1]) {
							var ocdt = obj["occur_dt"].replace(/(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})/g, '$1-$2-$3 $4:$5:$6');
							var tooltip_content = "<div class='tooltip_title' style='font-weight:bold;'>"+obj["title"]+"</div>"
												+"<div class='tooltip_loc' style='float:left;width:200px;'>"+obj["loc_nm"]+"</div>"
												+"<div class='tooltip_dt' style='float:right;width:150px;text-align:right;'>"+ocdt+"</div>";
							$("#ticker_tooltip").html( tooltip_content );
							$("#ticker_tooltip").attr("type", obj["type"]);
							var tooltipPos = ol.proj.fromLonLat( incidentObj["pos"] );
							ticker_tooltipPop = new ol.Overlay({
								element: document.getElementById("ticker_tooltip"),
								position: tooltipPos,
								positioning: 'bottom-left',
								offset: [20,20]
							});
							
							QvOSM_PVM_MAP.addOverlay(ticker_tooltipPop);
							QvOSM_PVM_MAP.getView().setCenter(tooltipPos);
						}*/
						...
						//draggable에 의한 위치 초기화
						//console.log('$curr_data',$curr_data);
						if("-" != incidentObj["pos"][0] && "-" != incidentObj["pos"][1] && "Y" == vTicker_Anl) {
							
							$("#ticker_pointPopupOnMap .ol-popup-headtitle-content").attr("title","");
							$("#ticker_pointPopupOnMap .ol-popup-headtitle-content").html("");
							$("#ticker_pointPopupOnMap #popup-content").html("");
							
							
							var ticker_pointPopupOnMap = new ol.Overlay({
								element: document.getElementById("ticker_pointPopupOnMap"),
								autoPan: true,
								autoPanAnimation: {
									duration: 250
								 }
							});
							QvOSM_PVM_MAP.addOverlay(ticker_pointPopupOnMap);
							var coordinate = ol.proj.fromLonLat( incidentObj["pos"] );
							$("#ticker_pointPopupOnMap").show();
							$("#ticker_pointPopupOnMap").addClass("incident");
							
							if(obj["type"]=="news"){ //news popup 최초 그리는 부분
								$("#ticker_pointPopupOnMap").attr("type",obj["type"]);
								$("#ticker_pointPopupOnMap .ol-popup-title").attr("type",obj["type"]);
								$("#ticker_pointPopupOnMap .ol-popup-headtitle-content").attr("title",obj["excp_nm"]);
								$("#ticker_pointPopupOnMap .ol-popup-headtitle-content").html(obj["excp_nm"]);
								...
								$("#ticker_pointPopupOnMap #popup-content").html(str);

								var ocdt = obj["occur_dt"].replace(/(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})/g, '$1-$2-$3 $4:$5:$6');
								var vdt = obj["valid_dt"].replace(/(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})/g, '$1-$2-$3 $4:$5:$6');
								$("#ticker_pointPopupOnMap #popup-content .occur_dt").html("Created on : "+ocdt);
								$("#ticker_pointPopupOnMap #popup-content .location").html(obj["loc_nm"]);
								$("#ticker_pointPopupOnMap #popup-content .title").html(obj["title"]);
								$("#pointPopup #popup-content .source").html("Source : "+obj["source"]);
								$("#ticker_pointPopupOnMap #popup-content .valid_dt").html("Valid until : "+vdt);
								//console.log( 'vdt :', vdt );
								if( "Y"== obj["anyFlag"] ){
									$("#ticker_pointPopupOnMap #popup-content .div_newsPop_img").html("<a target='_blank' href='"+QvOSM_exUrl+"imageHtml.html&qImageUrl="+obj["imgUrl"]+"'><img class='img_newsAddress' src='"+obj["imgUrl"]+"' style='width:57%; float:right;'></a>");
								}else{
									//$("#ticker_pointPopupOnMap #popup-content .div_newsPop_img").html("");
								}
								$("#ticker_pointPopupOnMap #popup-content .container_cnt").html((obj["container_cnt"] ? "Potentially Affected Cargos : "+obj["container_cnt"]+" container(s)" : ""));

								$("#ticker_pointPopupOnMap #popup-content .container_cnt").css("cursor","").css("text-decoration","").off("click");
								//console.log($curr_data);
								//console.log(obj["container_cnt"]);
								if(obj["container_cnt"]>0)
								$("#ticker_pointPopupOnMap #popup-content .container_cnt").css("cursor","pointer").css("text-decoration","underline").off("click").on("click",function(){
									//console.log("=====qlikview popup show===================================");
									//console.log($curr_data);

									//위치를 오른쪽으로 이동하여 왼쪽에 팝업과 나란히 배치한다. 시작
									$("#ticker_pointPopupOnMap").height("");
									$("#ticker_pointPopupOnMap").attr("style","");
									QvOSM_PVM_MAP.getView().setZoom(3);
									ticker_pointPopupOnMap.setPosition(coordinate);
									//위치를 오른쪽으로 이동하여 왼쪽에 팝업과 나란히 배치한다. 끝
									...
									//console.log('$curr_data',$curr_data);
									if("" == obj["newsVessel"]) {
										Qv.GetCurrentDocument().SetVariable("vID", obj["id"]);
										Qv.GetCurrentDocument().SetVariable("vLOC_CD", obj["loc_cd"]);
									} else {
										console.log('setval', obj["newsVessel"]);
										Qv.GetCurrentDocument().SetVariable("vNEWS_VESSEL", obj["newsVessel"]);
									}
									console.log("=====qlikview popup show===================================");
								});

								var $contents_arr = obj["cont"].split("[read more](");
								// console.log($contents_arr);
								...
								$("#ticker_pointPopupOnMap #popup-content .cont").html($contents+$read_more);

							}else{
								$("#ticker_pointPopupOnMap").attr("type",obj["type"]);
								$("#ticker_pointPopupOnMap .ol-popup-title").attr("type",obj["type"]);
								$("#ticker_pointPopupOnMap .ol-popup-headtitle-content").attr("title",obj["excp_nm4"]);
								$("#ticker_pointPopupOnMap .ol-popup-headtitle-content").html(obj["excp_nm4"]);
								...
								var ocdt = obj["occur_dt"].replace(/(\d{4})(\d{2})(\d{2})/g, '$1-$2-$3');
								$("#ticker_pointPopupOnMap #popup-content .occur_dt").html(ocdt);
								$("#ticker_pointPopupOnMap #popup-content .location").html(obj["loc_nm2"]+" ("+obj["step"]+")");
								$("#ticker_pointPopupOnMap #popup-content .title").html(obj["title"]);
								//$("#ticker_pointPopupOnMap #popup-content .vessel_nm").html(obj["vessel_nm"]);
								$("#ticker_pointPopupOnMap #popup-content .excp_nm4").html(obj["excp_nm4"]);
								$("#ticker_pointPopupOnMap #popup-content .reason").html((obj["reason"] ? obj["reason"] : ""));
								$("#ticker_pointPopupOnMap #popup-content .action").html((obj["action"] ? obj["action"] : ""));
								$("#ticker_pointPopupOnMap #popup-content .container_cnt").html((obj["container_cnt"] ? "Potentially Affected Cargos : "+obj["container_cnt"]+" container(s)" : ""));

							}
							ticker_pointPopupOnMap.setPosition([coordinate[0],coordinate[1]+300000]);
							QvOSM_PVM_MAP.getView().setCenter(coordinate);
					...
					$("#ticker_cont").html("There are no new exceptions registered within the last 7 days.");
					$('#ticker_cont').marquee('reset');
					$('#ticker_cont').marquee({
						speed: 80,
						gap: 0,
				...
				$('#ticker_cont').remove();
				$(".Document_vTICKER_COLOR .QvContent .TextObject TD:first").append('<div id="ticker_cont" ticker_step="-1" class="marquee" style="color:#ffffff;width:100%;height:40px;font-family:Calibri;font-size:24px;font-weight:bold;position:relative;left:0px;top:0px;"></div>');
				$.ticker_next();
				...
				$('#ticker_cont').off("mouseout").on("mouseout",function(evt){
					...
					setTimeout(function(){
						drawIncidentLayer();
						if( incidentIconAniobj.length > 0 ){
							for( var AniCnt = 0 ; AniCnt < incidentIconAniobj.length ; AniCnt++  ){
								fn_addAniFeature( incidentIconAniobj[AniCnt], AniCnt, 'news' );
							}
						}
						if( hotExcIconAniobj.length > 0 ){
							for( var AniCnt = 0 ; AniCnt < hotExcIconAniobj.length ; AniCnt++  ){
								fn_addAniFeature( hotExcIconAniobj[AniCnt], AniCnt, 'except' );
							}
						}
				...
				$('#ticker_cont').off("mouseover").on("mouseover",function(evt){
						...
						if($type=="news"){ //news popup 최초 그리는 부분
							...
							var str = "<table style='width:580px;'>"
								...
								+"<tr>"
									+"<td>"
										+"<div class='valid_dt'></div>"
										//+"<div><img style='height:30px;' src='http://totprd.pantos.com/gsi/cm/filemgnt/downloadIrreImgFile.img?imgFile=VANCOUVER[20190415140344].png&fileName=VANCOUVER.png'></div>"
									+"</td>"
								+"</tr>"
								+"<tr>"
									+"<td>"
							...
							//$("#ticker_pointPopup #popup-content .valid_dt").html("<img src='http://totprd.pantos.com/gsi/cm/filemgnt/downloadIrreImgFile.img?imgFile=VANCOUVER[20190415140344].png&fileName=VANCOUVER.png' style='height:30px;'>");
					...
					// 애니메이션 지우기
					if( incidentIconAni.length > 0 ){
						for( var i = 0 ; i < incidentIconAni.length ; i++ ){
							clearInterval(incidentIconAni[i]);
						}
						incidentIconAni = [];
					}
					if( hotExcIconAni.length > 0 ){
						for( var i = 0 ; i < hotExcIconAni.length ; i++ ){
							clearInterval(hotExcIconAni[i]);
						}
						hotExcIconAni = [];
					}
					// 다각형 layer 제거
					...
					if("-" != incidentObj["lngtd"])
						drawINCIDENT_tickerMouseover(incidentObj);
...
*/
			...
			function fn_addAniFeature( aniObj, num, type ){
				//window.setInterval( fn_drawAniFeature( incidentArr[0] ), 2000 );

				var pos;
				pos = new ol.Feature({
					geometry: new ol.geom.Point(ol.proj.fromLonLat([parseFloat(aniObj.lngtd),parseFloat(aniObj.ltitde)]))
				});

				fn_aniFlash( pos, num, type );
			}
			...
			var drawINCIDENT_tickerMouseover = function( tick ){
				//console.log(incidentData.length);
				var vIconFeas = [];
				if("string" == typeof tick) {
					$.each(incidentData,function($key,incidentObj){
						//console.log(incidentObj["except_arr"].length);
						//EXCT201906110004
						if(incidentObj["except_arr"].length>0) {
							var except_arr = [];
							for(var i=0, except; except=incidentObj["except_arr"][i]; i++) {
								//console.log(except.except_no);
								if(tick == except.relation){
									except_arr.push(except);
									break;
								}
							}
							if(except_arr.length > 0){
								//incidentObj["except_arr"] = except_arr;
								var vIconFea = drawINCIDENT_icon(incidentObj);
								vIconFeas.push(vIconFea);
							}
							//console.log(vIconFeas);
						...
						if( incidentObj["news_arr"].length>0 ) {
							for(var i=0, news; news=incidentObj["news_arr"][i]; i++) {
								if("Y" == news.anyFlag && tick == news.id)	{
									var vIconFea = drawINCIDENT_icon(incidentObj);
									vIconFeas.push(vIconFea);
								}
							}
					...
					$.each(incidentData,function($key,incidentObj){
						if( incidentObj["news_arr"].length>0 && tick["news_arr"].length>0) {
							for(var i=0, news; news=incidentObj["news_arr"][i]; i++) {
								if(tick["news_arr"][0].news_no == news.news_no)	{
									resultBool = true;
									break;
								}
							}
						} else if( incidentObj["except_arr"].length>0 && tick["except_arr"].length>0) {
							for(var i=0, except; except=incidentObj["except_arr"][i]; i++) {
								if(tick["except_arr"][0].except_no == except.except_no) {
									resultBool = true;
									break;
								}
							}
			...
			var checknewzoom = function(evt) {
			   ...
			   {
				    if(!lockBoolean && ((4 > currentZoomLevel && 4 <= newZoomLevel) || (4 <= currentZoomLevel && 4 > newZoomLevel)))
						drawINCIDENT();
			...
			var playAudio = function() {
				...
				$('#playAudio').remove();
				if(!isChrome){
					var playAudio = '<audio autoplay  id="playAudio">'
								+'<source src="http://qvdb.lxpantos.com/sso/exception.mp3">'
								+'</audio>';
				}
				else{
					var playAudio = '<iframe src="http://qvdb.lxpantos.com/sso/exception.mp3" allow="autoplay" style="display:none" id="playAudio"></iframe>';
				}
				$("body").append(playAudio);
			}
			...
			Qva.AddExtension(ExName, function() {
				...
				if(!varsRetrieved){
                    ...
                    if( vEXT_INCIDENT.Data.Rows.length ){
						...
						if( vEXT_INCIDENT.GetVariable(14).text ){
                            try{
                                //vToolTip_YN
                                if( '-' != vEXT_INCIDENT.GetVariable(14).text && '' != vEXT_INCIDENT.GetVariable(14).text && null != vEXT_INCIDENT.GetVariable(14).text ){
                                    vToolTip_YN = vEXT_INCIDENT.GetVariable(14).text;
                                }else{
                                    vToolTip_YN = 'OFF'; //없으면 OFF
                                }
                                //console.log( 'vToolTip_YN : ', vToolTip_YN );
                                // console.log( 'vVESSEL_OBJ.length : ', vVESSEL_OBJ.length );
                            }catch( e ){
                                //IsChkCnt++;
                                console.log( "can't get vToolTip_YN data : ", e );
                            }
                        }
						
						if( vEXT_INCIDENT.GetVariable(15).text ){
                            try{
                                //vNEWS_EXCEPT_LV1_2
                                if( '-' != vEXT_INCIDENT.GetVariable(15).text && '' != vEXT_INCIDENT.GetVariable(15).text && null != vEXT_INCIDENT.GetVariable(15).text ){
                                    vNEWS_EXCEPT_LV1_2 = vEXT_INCIDENT.GetVariable(15).text;
                                }else{
                                    vNEWS_EXCEPT_LV1_2 = '0'; //없으면 0
                                }
                                //console.log( 'vNEWS_EXCEPT_LV1_2 : ', vNEWS_EXCEPT_LV1_2 );
                            }catch( e ){
                                //IsChkCnt++;
                                console.log( "can't get vToolTip_YN data : ", e );
                            }
						...
						if( vEXT_INCIDENT.GetVariable(16).text ){
                            try{
                                //vTicker_Anl
                                if( '-' != vEXT_INCIDENT.GetVariable(16).text && '' != vEXT_INCIDENT.GetVariable(16).text && null != vEXT_INCIDENT.GetVariable(16).text ){
                                    vTicker_Anl = vEXT_INCIDENT.GetVariable(16).text;
                                }else{
                                    vTicker_Anl = 'N'; //없으면 0
                                }
                                //console.log( 'vNEWS_EXCEPT_LV1_2 : ', vNEWS_EXCEPT_LV1_2 );
...
//								console.log(list);
						...
						if( IconAni.length > 0 ){
							for( var i = 0 ; i < IconAni.length ; i++ ){
								clearInterval(IconAni[i]);
							}
							IconAni = [];
						}
						IconAniobj = [];
						...
						$.each(vNewsObj,function($key,$value){
							...
							var imgUrl = "";
							var anyFlag 		= ($value["ANI_FLAG"])									   		?	$value["ANI_FLAG"]	        	    :	"";//ANI_FLAG;
							var newsMinLocYn 	= ($value["NEWS_MIN_LOC_YN"])								    ?	$value["NEWS_MIN_LOC_YN"]	   	    :	"";//NEWS_MIN_LOC_YN;
							var newsVessel 		= ($value["NEWS_VESSEL"])									    ?	$value["NEWS_VESSEL"]	 	  	    :	"";//newsVessel;
							/*if(anyFlag == "Y")
								console.log('container_cnt',container_cnt);
							if( newsVessel != ""){
								container_cnt = "1";
							}*/
							//anyFlag Y일경우, popup에 img를 뿌려준다.
							if( $value["ANI_FLAG"] == 'Y' ){
							...
							var newsObj = {
									...
									newsVessel		:	newsVessel
							};


							if( newsObj.anyFlag == "Y" ){
								//console.log('newsObj :',newsObj);
								//console.log('incidentIconAniobj :',incidentIconAniobj);
								incidentIconAniobj.push( newsObj );
								//console.log(11);
							}
							
							if( exc_btn_bool && exc_btn_id && exc_btn_id == news_no ) {
								IconAniobj.push(newsObj);
							}
							...
							$.each(incidentData,function($k,$v){
								if($v["news_cnt"]>0 && newsObj["lngtd"]==$v["lngtd"] && newsObj["ltitde"]==$v["ltitde"]
									...
									if(incidentData[$k]["news_arr"].length==1 && $v["news_cnt"]>0 && $v["except_cnt"]==0 && (vNEWS_ONOFF=="ON" || vNEWS_EXCEPT_LV1_2=="1")){// && vEXCEPTION_ONOFF=="OFF"
										//console.log( newsObj );
										//switch(newsObj["incident_type2"].trim()){
										switch( newsObj["excp_cd"] ){
										...
										case "EXE_0043":
											...
											break;
										case "EXE_0056":
											incidentData[$k]["icon"] = "FireOnShip_T";
						...
						$.each(vExceptObj,function($key,$value){
							...
							var excp_cd			= ($value["EXCP_CD"])									?	$value["EXCP_CD"]	        :	"";//excp_cd
							var level			= ($value["LEVEL"])										?	" Lv. "+$value["LEVEL"]	    :	"";//level
							var relation		= ($value["RELATION"])									?	$value["RELATION"]	   		:	"";//RELATION
							var hotFlag			= ($value["HOT_FLAG"])									?	$value["HOT_FLAG"]	   		:	"";//HOT_FLAG
							var tickerFlag		= ($value["TICKER_FLAG"])								?	$value["TICKER_FLAG"]	   	:	"";//TICKER_FLAG
							var alarmFlag		= ($value["ALARM_FLAG"])								?	$value["ALARM_FLAG"]	   	:	"";//ALARM_FLAG
							...
							/*if(relation=="9064004")
								console.log('relation',except_no);*/
							...
							if( exceptObj.hotFlag == "Y" ) {
								hotExcIconAniobj.push( exceptObj );
							}
							if(exc_btn_bool && exc_btn_id && exc_btn_id == relation && exceptObj.hotFlag == "Y") {
								IconAniobj.push( exceptObj );
							}
							...
							$.each(incidentData,function($k,$v){
								if($v["except_cnt"]>0 && exceptObj["lngtd"]==$v["lngtd"] && exceptObj["ltitde"]==$v["ltitde"]
									...
									//console.log("======================");
									//return true;
									if( (vEXCEPTION_ONOFF=="ON" || vNEWS_EXCEPT_LV1_2=="1") ) {
										switch(exceptObj["level"]){
											case " Lv. 1":
												if("Y" == exceptObj["hotFlag"] && "ELV1_NEW" != incidentData[$k]["icon"]) {
													if("Y" == exceptObj["alarmFlag"])
														incidentData[$k]["icon"] = "ELV1_NEW";
													else
														incidentData[$k]["icon"] = "ELV1_T";
												} else if("ELV1_T" != incidentData[$k]["icon"] || "ELV1_NEW" != incidentData[$k]["icon"]) {
													incidentData[$k]["icon"] = "ELV1";
												}
												incidentData[$k]["color"] = "rgba(223, 0, 36, 1)";
												incidentData[$k]["news_on"] = 1;
												break;
											case " Lv. 2":
												if("Y" == exceptObj["hotFlag"] && "ELV2_NEW" != incidentData[$k]["icon"]) {
													if("Y" == exceptObj["alarmFlag"])
														incidentData[$k]["icon"] = "ELV2_NEW";
													else
														incidentData[$k]["icon"] = "ELV2_T";
												} else if("ELV2_T" != incidentData[$k]["icon"] || "ELV2_NEW" != incidentData[$k]["icon"]) {
													incidentData[$k]["icon"] = "ELV2";
												}
												incidentData[$k]["color"] = "rgba(255, 0, 0, 1)";
												incidentData[$k]["news_on"] = 1;
												break;
										}
									}
								
								//console.log(incidentData[$k]["icon"]);
						...
						if(IsChkCnt>0){
							...
							setTimeout(function(){
								...
								$("#ticker_pointPopupOnMap").hide();
								if( alwaysTooltipOverlays.length > 0 ){
									//overlay 지우기
									for( var overlayCnt = 0 ; overlayCnt < alwaysTooltipOverlays.length ; overlayCnt++ ){
										QvOSM_PVM_MAP.removeOverlay( alwaysTooltipOverlays[overlayCnt] );
									}
								}
								lockBoolean=false;
								//console.log('exc_btn_id',exc_btn_id);
								if(exc_btn_bool && exc_btn_id) {
									// 다각형 layer 제거
									for( var k = 0 ; k < polygonLayers.length ; k++ ){
										QvOSM_PVM_MAP.removeLayer(polygonLayers[k]);
									}
									if( IconAniobj.length > 0 ){
										for( var AniCnt = 0 ; AniCnt < IconAniobj.length ; AniCnt++  ){
											fn_addAniFeature( IconAniobj[AniCnt], AniCnt, 'incident' );
										}
									}
									drawINCIDENT_tickerMouseover(exc_btn_id);
									//exc_btn_bool = false;
								} else {
									drawINCIDENT();
								}
								...
								//
								if('OFF' == vTicker)
									QvOSM_PVM_MAP.removeOverlay(ticker_tooltipPop);
