			...
			try {
				...
				if(!this.framecreated){
					...
					*//*<img src="'+QvOSM_PVM_exUrl+'icon/btn_transit.png" width="89" height="27"/>*/
					htmlString += '<div id="pointPopup" class="ol-popup"><div class="ol-popup-title"><span class="ol-popup-headtitle-content">Vessel</span><span class="ol-popup-title-content"></span></div><a href="#" id="popup-closer" class="ol-popup-closer"></a><div id="popup-content"></div></div><div id="geo-marker"></div>'+
					...
					for(var tyKey in typhoonData.tyLine){
						...
						if( tyLineLength > 1){
							...
							for(;tyL_i<tyLineLength-1;tyL_i++){
								...
								if( tyLine[tyL_i].point[0] * tyLine[tyL_i+1].point[0] < 0 ){							
									var beforePoint = tyLine[tyL_i].point;
									var afterPoint = tyLine[tyL_i+1].point;
									
									if( afterPoint[0] < 0  ){
										afterPoint[0] = 360 + afterPoint[0];
									}else{
										afterPoint[0] = -360 + afterPoint[0];
									}
									
									var coordinates = [ol.proj.fromLonLat( beforePoint ), ol.proj.fromLonLat( afterPoint )];
								}else{
				...
				/*$(".vessel_name_box").on("click",function(){
